...
** Last update Sun Mar  5 15:08:12 2017 Benjamin GAYMAY
*/
...
int	launch_archive(int argc, char **argv, char *flag)
{
  int	lines;
  int	fd_file;

  fd_file = open(argv[2], O_CREAT | O_WRONLY | O_TRUNC, 0644);
  if (fd_file == -1)
    return (ERROR);
  while (--argc > 2)
    {
      if (strcmp(argv[argc], argv[2]) == 0)
	return (ERROR);
